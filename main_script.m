...
%'Home'    
Matlab_trading_simulations_folder='C:\Users\Xiangnan\Dropbox\zhu\FIN566';
...
%(This will need to be modified on each student's code)
matching_engine_algo='engine_jia';


%% %*****Designating the Orderbook-Construction Script's Name*****
%(This will need to be modified on each student's code)
orderbook_construction_code='order_book_xiang';
...
background_trader_commands='trader_zhu';

%% %*****Designating Post-Simulation Output-Analysis Script's Name*****
%(This will need to be modified on each student's code)
sim_output_analysis_code='analysis_zhu';
...
live_buy_orders_list=zeros(1,7);
live_sell_orders_list=zeros(1,7);
...
% Storing transaction information
    % time, aggressor sign(buy+1/sell-1), price, executed quantity, passor order_id,passor_account_id, aggressor_account_id
...
eval('order_book_xiang2')


toc

transaction_price_volume_stor_mat(1,:)=[]; %Removing the row of ones 
live_sell_orders_list(1,:) = [];
live_buy_orders_list(end,:) = [];
